...
const UserProfile = (props) => {
  ...
  useEffect(() => {
    axios.get(`http://api.ratemyshow.lekiam.net/users/${params.username}`).then((response) => {
  ...
  return (
    <div className="userprofile-container">
      <div className="userprofile-profiledata">
        {isLoading ? <div></div> : <img src={image} alt="Foto de perfil" className="userprofile-image" />}
      ...
      <div className="userprofile-fav-pending">
        <div>
          <h1>FAVORITAS</h1>
          <div className="userprofile-fav">{userProfile.favorites.map(recommendationsToComponent).slice(0, 5)}</div>
        </div>
        <div>
          <h1>PENDIENTES</h1>
          <div className="userprofile-pending">{userProfile.pending.map(recommendationsToComponent).slice(0, 5)}</div>
        </div>
      </div>
    </div>
  );
